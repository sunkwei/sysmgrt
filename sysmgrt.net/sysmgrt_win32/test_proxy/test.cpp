...
int main()
{
	...
	zksysmgrt_Host *services = 0;
	int scnt = 0;

	int rc = zksysmgrt_q_getallhosts(sm, 1, &services, &scnt);
	zksysmgrt_q_freehosts(sm, services, scnt);
	...
	for (int i = 0; i < 100; i++) {
		...
		rc = zksysmgrt_r_reghost(sm, name, "", (const char**)url, 1);
	}

	for (int i = 0; i < 1000000; i++) {
		rc = zksysmgrt_q_getallhosts(sm, 1, &services, &scnt);
		if (rc != 0) {
